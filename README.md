...
| `step37-flash-nvfp4-tp2.env` | stepfun-ai/Step-3.7-Flash-NVFP4 | NVFP4 (ModelOpt) + FP8 KV | dual-rdma | 2 | **v022-d568-step3p7-memcheck-bypass** (derived layer) | **EXPERIMENTAL** — Stages A–C validated (EP-on + mp, CUDA graph, up to 262k startup and short inference). Stage D (long-context ladder) stopped at D0: reasoning model exhausted 128/256 output token budget before completing marker strings (retrieval was correct). Multi-sequence unvalidated. Requires bypass image + clean reboot (≥ 110 GiB/node). See [`docs/benchmarks/step37-v022-long-context-validation.md`](docs/benchmarks/step37-v022-long-context-validation.md). **For production use `step37-flash-nvfp4-v023-tp2-latency.env`.** |
