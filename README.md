...
| `step37-flash-nvfp4-tp2.env` | stepfun-ai/Step-3.7-Flash-NVFP4 | NVFP4 (ModelOpt) + FP8 KV | dual-rdma | 2 | **v022-d568-step3p7-memcheck-bypass** (derived layer) | **EXPERIMENTAL long-context path — `STAGE_D_PARTIALLY_VALIDATED_TO_245009`**: Stages A–C and Stage D single-sequence requests through **245,009 prompt tokens** (D0–D4) are runtime-validated from a clean-memory precondition (EP-on + mp + CUDA graph). A **257,891-token** prompt (`D5_INFRASTRUCTURE_HANG_AT_257891`) reproducibly caused an infrastructure hang; retrieval correctness at that depth was not evaluated. Multi-sequence unvalidated. Requires bypass image + ≥ 110 GiB pre-start preflight per node. Do not issue requests above 245,009 tokens. See [`docs/benchmarks/step37-v022-long-context-validation.md`](docs/benchmarks/step37-v022-long-context-validation.md). |
