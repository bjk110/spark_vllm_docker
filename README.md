...
| `step37-flash-nvfp4-tp2.env` | stepfun-ai/Step-3.7-Flash-NVFP4 | NVFP4 (ModelOpt) + FP8 KV | dual-rdma | 2 | **v022-d568-ngc2605-tx5102-vllm022-step3p7-modelopt-cache-release** | Requires `patches/patch_step3p7_nvfp4_input_scale.py` (missing `expert_params_mapping.input_scale` causes NaN logits otherwise) and `patches/patch_step3p7_modelopt_cache_release.py` (prevents cumulative CUDA caching-allocator reserved-memory growth during ModelOpt NVFP4 MoE MARLIN post-load conversion; feature-gated via `VLLM_SPARK_EMPTY_CACHE_AFTER_MODELOPT_MOE`). `MAX_NUM_SEQS=4` verified. Post-shutdown UMA residual (~19 GiB) on GB10 requires reboot to recover. Full guide: [`docs/step3.7-flash-tp2.md`](docs/step3.7-flash-tp2.md). Memory-freeze diagnostic: [`docs/diagnostics/dgx-spark-uma-memory-freeze.md`](docs/diagnostics/dgx-spark-uma-memory-freeze.md). |
