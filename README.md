...
| `step37-flash-nvfp4-tp2.env` | stepfun-ai/Step-3.7-Flash-NVFP4 | NVFP4 (ModelOpt) + FP8 KV | dual-rdma | 2 | **v022-d568-ngc2605-tx5102-vllm022-step3p7-modelopt-cache-release** | **EXPERIMENTAL** — long-context (262k) multiprocessing-backend exploration profile. EP-on, mp backend, MAX_NUM_SEQS=4, MAX_MODEL_LEN=262144 (model native). Runtime-unvalidated: EP + mp + dual-node path has not been tested end-to-end; CUDA-graph warmup at 262k with 4 sequences may exhaust UMA. **For validated production serving use `step37-flash-nvfp4-v023-tp2-latency.env` instead.** Full guide: [`docs/step3.7-flash-tp2.md`](docs/step3.7-flash-tp2.md). |
