...
| `step37-flash-nvfp4-tp2.env` | stepfun-ai/Step-3.7-Flash-NVFP4 | NVFP4 (ModelOpt) + FP8 KV | dual-rdma | 2 | **v022-d568-step3p7-memcheck-bypass** (derived layer) | **EXPERIMENTAL — `STAGE_D_PARTIALLY_VALIDATED_TO_245009`**: Stages A–C and Stage D single-sequence requests through **245,009 prompt tokens** (D0–D4) are runtime-validated from a clean-memory precondition (EP-on + mp + CUDA graph). A near-258k single request (`D5_INFRASTRUCTURE_HANG_AT_257891`) reproducibly caused an infrastructure hang under the tested configuration. Retrieval correctness at that depth was not evaluated. Multi-sequence operation remains unvalidated. Requires bypass image + clean reboot (≥ 110 GiB/node). See [`docs/benchmarks/step37-v022-long-context-validation.md`](docs/benchmarks/step37-v022-long-context-validation.md). **For production use `step37-flash-nvfp4-v023-tp2-latency.env`.** |
| `step37-flash-nvfp4-v023-tp2-latency.env` | stepfun-ai/Step-3.7-Flash-NVFP4 | NVFP4 (ModelOpt) + FP8 KV | dual-rdma | 2 | **v023-step3p7-fixed-kv-profile-skip-candidate** | vLLM 0.23.0 Series A latency-oriented configuration. EP disabled, MARLIN MoE backend (required for correctness on SM_121 in v0.23), TRITON_ATTN (required — FlashInfer reasoning garble on SM_121), CUDA graph disabled, `MAX_NUM_SEQS=1`, `MAX_NUM_BATCHED_TOKENS=2048` (preferred measured value). bt=8192 was measured and remained valid, but it was not preferred for the tested single-sequence latency workload because its small deep-prefill gain (+3–4% at d4096+) did not outweigh lower measured short-context decode throughput (−6–18%). Benchmark: [`docs/benchmarks/bt-matrix-step37-nvfp4-v023.md`](docs/benchmarks/bt-matrix-step37-nvfp4-v023.md). |
...
│   ├── step37-flash-nvfp4-tp2.env    # Step-3.7-Flash NVFP4 v0.22, EP-on, 262k (EXPERIMENTAL; runtime-unvalidated)
│   ├── step37-flash-nvfp4-v023-tp2-latency.env  # Step-3.7-Flash NVFP4 v0.23 latency preset (EP-off, MARLIN, bt=2048)
