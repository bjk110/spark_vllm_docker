...
| `step37-flash-nvfp4-tp2.env` | stepfun-ai/Step-3.7-Flash-NVFP4 | NVFP4 (ModelOpt) + FP8 KV | dual-rdma | 2 | **v022-d568-step3p7-memcheck-bypass** (derived layer) | **EXPERIMENTAL** — Stage A (eager, 32k, seq1), Stage B (CUDA graph, 32k, seq1), and Stage C (CUDA graph, 262k, seq1) validated with EP-on + mp backend. Requires `v022-d568-step3p7-memcheck-bypass` image (adds three UMA init-check bypass patches) + clean reboot (≥ 110 GiB MemAvailable per node) before start. Multi-sequence operation remains unvalidated. See [`docs/benchmarks/step37-v022-long-context-validation.md`](docs/benchmarks/step37-v022-long-context-validation.md). **For validated production serving use `step37-flash-nvfp4-v023-tp2-latency.env`.** |
