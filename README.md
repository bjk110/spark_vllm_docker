...
| `step37-flash-nvfp4-tp2.env` | stepfun-ai/Step-3.7-Flash-NVFP4 | NVFP4 (ModelOpt) + FP8 KV | dual-rdma | 2 | **v022-d568-ngc2605-tx5102-vllm022-step3p7-modelopt-cache-release** | **EXPERIMENTAL / BLOCKED** — long-context (262k) multiprocessing-backend profile. EP-on, mp backend, MAX_NUM_SEQS=4, MAX_MODEL_LEN=262144 (model native). **Cannot start on GB10 driver 610**: image missing `patch_skip_init_memory_check.py`; CUDA free (~50 GiB) < GPU_UTIL×total (107 GiB) → `request_memory()` fails at startup. Fix: rebuild image with bypass patches + `VLLM_SKIP_INIT_MEMORY_CHECK=1`. See [`docs/benchmarks/step37-v022-long-context-validation.md`](docs/benchmarks/step37-v022-long-context-validation.md). **For validated production serving use `step37-flash-nvfp4-v023-tp2-latency.env`.** |
