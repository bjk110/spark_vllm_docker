...
lines.append("| TP | 2 |")
lines.append("| EP | **Depends on series** — see metadata.txt `expert_parallel_requested`/`expert_parallel_observed` per run. bt=256 confirmed EP-off; bt-matrix-base.env runs are EP-on. Cross-series comparison is NOT a pure bt variable test. |")
...
lines.append("")
lines.append("1. A 2048-token prompt at bt=256 results in up to 8 sequential scheduler")
lines.append("   iterations (expected from budget math; iteration count not instrumented).")
lines.append("2. Step-3.7 has 288 MoE experts (top-8 routing). With 256-token chunks,")
lines.append("   average routed token assignments per expert ≈ 256×8/288 ≈ 7.1 per chunk.")
lines.append("   MARLIN grouped GEMM efficiency degrades at low row counts. This is a")
lines.append("   plausible mechanism; GEMM row utilization was not directly profiled.")
lines.append("3. TP all-reduce and local expert dispatch overhead repeats per scheduler")
lines.append("   iteration (EP was disabled in the bt=256 run — TP-only synchronization).")
lines.append("   This is a plausible contributor; overhead was not isolated from other factors.")
lines.append("")
...
lines.append("|---------|------------|")
lines.append("| bt=256 is associated with ~57% prefill regression vs v022 baseline | **Observed** (538 t/s measured, v022 1251 t/s — different images, different EP state — not isolated) |")
lines.append("| Higher bt values are expected to improve prefill | **Indicated** (v022 at bt~8192 showed 1251 t/s; v023 unmeasured; EP state differs) |")
lines.append("| MARLIN row starvation is the mechanism | **Plausible** (not profiled; GEMM row count is inferred, not observed) |")
lines.append("| TRITON_ATTN adds prefill overhead vs FlashInfer | **Possible contributor** (not isolated; FlashInfer garbles on SM_121 in v023 — cannot remove it to isolate) |")
lines.append("| v023 per-se adds prefill overhead | **Possible contributor** (not isolated) |")
lines.append("| bt=2048 is the single-chunk boundary for pp=2048 | **Expected from budget math** (not yet measured on v023) |")
lines.append("| decode throughput unaffected by bt | **Indicated** (bt=256 tg32 ~12.3 t/s; v022 ~13.4 t/s — small gap) |")
lines.append("| bt≥8192 is safe and stable on GB10 + UMA + v023 | **Not tested** |")
lines.append("")

lines.append("## Phase 7: Production candidates (not validated)")
lines.append("")
lines.append("> **Status: Not validated.** No production MAX_NUM_BATCHED_TOKENS value has been")
lines.append("> confirmed for the v023 correctness-safe configuration (MARLIN + TRITON_ATTN).")
lines.append("> The table below lists provisional candidates for measurement, not validated recommendations.")
lines.append("")
lines.append("| Candidate | bt | Rationale | Risk |")
lines.append("|-----------|---:|-----------|------|")
lines.append("| Conservative candidate | 4096 | 2x pp2048 chunk size; safe headroom | Moderate -- may not fully recover prefill |")
lines.append("| Provisional starting candidate | 8192 | v022 production default (different EP state); not yet measured on v023 | Unknown -- memory and throughput impact on v023 unmeasured |")
...
lines.append("")
lines.append("> Each candidate requires a correctness validation run before any production consideration.")
lines.append("> Do not apply to presets/step37-flash-nvfp4-tp2.env until full v023 validation complete.")
lines.append("")

lines.append("### Provisional candidate test diff (disposable env only)")
lines.append("")
