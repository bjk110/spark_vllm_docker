...
| `sm121/apply_sm121_rowwise_mqa_graph_safe.py` | SM121 / runtime | Standby | Fixes `EngineDeadError` at long context (observed at >256K; threshold-governed, see report): de-constexprs the 19 runtime-variant params of `_fp8_paged_mqa_logits_rowwise_kernel` so novel context shapes stop JIT-loading cubins inside FULL_DECODE_ONLY CUDA graph captures. Validated dual-GB10 TP=2 @ 512K max-model-len. An experimental active-validation image is published (`ghcr.io/bjk110/vllm-spark:h1z-p50-sm121-rowwise-mqa-graphsafe-exp-41d211f`, digest `sha256:4b8f650aa96e…`; one-file runtime patch, P50B/P50C validated to 256K) but **production wiring remains none**. See `docs/deepseek-v4-sm121-rowwise-mqa-cudagraph-fix.md` |
